...
- A view class should not contain any logic. It just renders a single element of the game state. Examples: Health bar, Coin HUD, a Character avatar, an item in the inventory, a card in a deck, etc.
- A presenter class is an aggregator of view objects. It may know positions, transitions, states, and animations. Usually, there is a presenter for each game scene (Gameplay, Menu, Character Selection, etc.).
