...
4. If we had a Milestone Change or Sprint Change, read `.docs/backlog.md`.
  - If the backlog is not found, create it with the template in `.agents/docs/backlog-template.md`. Then it is a fresh backlog.
  - If the backlog is not fresh and no tasks are defined for current sprint:
    - Get user permission with `AskUserQuestion` to clean it up.
    - Run an Interview to define what are the tasks for current sprint.
  - Do not proceed until the user confirms the tasks.
  - Update `.docs/backlog.md` with the confirmed tasks for current sprint.

5. If we did not have a Milestone Change or Sprint Change and there are tasks for current sprint, run the skill `/daily` and continue from there.
...
- This skill does 3 things:
  ...
  - Changes current milestone and current sprint when the user confirms that they have reached that milestone or want to change the current sprint focus.
